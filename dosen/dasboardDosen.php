...
// Ambil data dosen dari tb_user (tidak terlalu dipakai di tampilan, tapi bagus untuk ada)
$result = mysqli_query($konek, "SELECT * FROM tb_user WHERE username = '$username' AND role = 'dosen'");
$dosen_data = mysqli_fetch_assoc($result);
...
mysqli_stmt_bind_param($stmt_course, "s", $username);
mysqli_stmt_execute($stmt_course);
$course_query = mysqli_stmt_get_result($stmt_course);

// Data pengumuman (bisa diambil dari database di masa depan, contoh statis dulu)
$pengumuman = [
    ["judul" => "Pengumuman Pembukaan Semester Genap", "tanggal" => "10 Juli 2025"],
    ["judul" => "Batas Akhir Input Nilai Mahasiswa", "tanggal" => "25 Juli 2025"],
    ["judul" => "Rapat Koordinasi Dosen", "tanggal" => "1 Agustus 2025"]
];
...
<head>
    ...
    <style>
        ...
        /* --- Course Card Styling (Diperbarui untuk dosen) --- */
        .course-card {
            border-radius: 15px;
            border: 1px solid var(--border-color);
        ...
        @media (min-width: 768px) and (max-width: 991px) {
            ...
            .col-lg-4 {
                width: 50%;
                /* Override Bootstrap default for 3 columns on lg, force 2 columns on tablets */
            }
        }
    ...
    <div class="wrapper" id="wrapper">
        <div class="sidebar">
            ...
            <div class="nav-link-list">
                <a href="dashboardDosen.php" class="<?= (basename($_SERVER['PHP_SELF']) == 'dashboardDosen.php' && !isset($_GET['course_id'])) ? 'active' : ''; ?>">
        ...
        <div class="content">
            ...
            <div class="container-fluid px-4">
                <div class="dashboard-header">
                    ...
                    <h7>Kelola mata kuliah dan aktivitas mengajar Anda.</h7>
                </div>

                <h5>Mata Kuliah Mahasiswa</h5>
                ...
                <div class="row g-4">
                    <?php if (mysqli_num_rows($course_query) > 0) : ?>
                        <?php while ($course = mysqli_fetch_assoc($course_query)) : ?>
                            <div class="col-12 col-sm-6 col-md-6 col-lg-4 col-xl-4">
                                <div class="card course-card">
                                    <img src="<?= !empty($course['gambar']) ? '../' . htmlspecialchars($course['gambar']) : '../assets/images/default.jpg' ?>"
                                        class="card-img-top" alt="Course Image" style="height: 200px; object-fit: cover;">
                                    <div class="card-body">
                                        <h5 class="card-title"><?= htmlspecialchars($course['kode_mk']) ?> - <?= htmlspecialchars($course['nama_mk']) ?></h5>
                                        <p class="card-text"><small>Semester: <?= $course['semester'] ?></small></p>
                                        <p class="card-text"><small>SKS: <?= $course['sks'] ?></small></p>
                                        <p class="card-text"><small>Dosen: <?= htmlspecialchars($course['dosen']) ?></small></p>
                                    </div>
                                    <div class="card-footer">
                                        <a href="kelola_course.php?id=<?= $course['id']; ?>" class="btn btn-outline-primary">Kelola Mata Kuliah</a>
                                        <div class="dropdown">
                                            <button class="btn btn-light" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                                <i class="bi bi-three-dots-vertical"></i>
                                            </button>
                                            <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0">
                                                <li><a class="dropdown-item" href="manage_materi.php?course_id=<?= $course['id']; ?>">Kelola Materi</a></li>
                                                <li><a class="dropdown-item" href="manage_tugas.php?course_id=<?= $course['id']; ?>">Kelola Tugas</a></li>
                                                <li><a class="dropdown-item" href="manage_absensi.php?course_id=<?= $course['id']; ?>">Kelola Absensi</a></li>
                                                <li>
                                                    <hr class="dropdown-divider">
                                                </li>
                                                <li><a class="dropdown-item text-danger" href="#">Arsip Mata Kuliah</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <?php else : ?>
                        <div class="col-12">
                            <div class="empty-state">
                                ...
                                <h5>Belum Ada Mata Kuliah Anda Ampu</h5>
                                <p>Silakan tambahkan mata kuliah yang Anda ampu atau hubungi admin.</p>
                            </div>
                        </div>
                    <?php endif; ?>
                </div>
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="rightPanel" aria-labelledby="rightPanelLabel">
            ...
            <div class="offcanvas-body">
                ...
                <div class="alert alert-info d-flex align-items-center mb-3" role="alert">
                    ...
                    <div>Selamat datang di dashboard Dosen!</div>
                </div>
                <div class="alert alert-warning d-flex align-items-center" role="alert">
                    <i class="bi bi-exclamation-triangle-fill me-2 fs-5"></i>
                    <div>Periksa jadwal pengisian nilai.</div>
                </div>
            </div>
        </div>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleSidebar() {
                ...
                if (wrapper.classList.contains('collapsed')) {
                    localStorage.setItem('sidebarCollapsedDosen', 'true'); // Ubah key untuk dosen
                } else {
                    localStorage.removeItem('sidebarCollapsedDosen');
                }
            }

            // Memuat status collapsed dari localStorage saat halaman dimuat
            if (localStorage.getItem('sidebarCollapsedDosen') === 'true') { // Ubah key untuk dosen
